# Hi, I'm Muhammad Azam 👋

Full Stack Developer focused on building scalable real-world software solutions.

🌐 Portfolio: https://azamdeveloper.com/  
📍 Based in Pakistan  
💼 Open to Remote Opportunities  

---

## 🚀 About Me

I design and develop full-stack applications that solve real administrative and business problems.

My focus is on:

- Scalable backend architecture
- Clean database design
- Timeline-based data systems
- Practical government & office workflow solutions

---

## 🛠 Tech Stack

### Frontend
- React
- JavaScript
- Bootstrap

### Backend
- PHP
- REST APIs
- MySQL

### Tools
- Git & GitHub
- XAMPP
- VS Code

---

## 🔥 Highlight Project

### 🏛 Judiciary HRMS

A production-ready Human Resource Management System built for judiciary / government workflow.

✔ Timeline-based employee history tracking  
✔ Transfer & posting management  
✔ District / Tehsil posting logic  
✔ Court vs Office differentiation  
✔ Live deployed system  

🔗 Live Demo: https://azamdeveloper.com/  
🔗 Frontend Repository: https://github.com/azamwebdesigner-dotcom/judiciary-hrms-frontend



---

## 🎯 Development Philosophy

I don’t just build CRUD applications.  
I design systems that preserve data history and reflect real-world workflows.

---


---

## 📊 GitHub Stats

![Azam's GitHub stats](https://github-readme-stats.vercel.app/api?username=azamwebdesigner-dotcom&show_icons=true&theme=default)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=azamwebdesigner-dotcom&layout=compact)
## 📫 Connect With Me

🌐 Website: https://azamdeveloper.com/  
💼 LinkedIn: 🔗 https://www.linkedin.com/in/mohammad-azamali/
